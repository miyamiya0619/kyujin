...
@section('content')
    <section class="mx-auto max-w-3xl px-4 py-8">
            ...
            <div class="flex items-center gap-4">
                @if ($company->logo_path)
                    <img src="{{ \App\Services\ImageUploadService::url($company->logo_path) }}"
                         alt="{{ $company->name }}" class="h-16 rounded border border-[var(--border)] bg-[var(--surface)] p-1">
                @endif
                <h1 class="text-xl font-bold">{{ $company->name }}</h1>
            </div>

            @if ($company->description)
                <p class="mt-4 whitespace-pre-line text-sm text-[var(--ink-soft)]">{{ $company->description }}</p>
            @endif
        ...
        @if ($workplaces->isNotEmpty())
            <div class="mt-8">
                <h2 class="text-sm font-semibold text-[var(--ink-soft)]">事業所</h2>
        ...
        <div class="mt-8">
            <h2 class="text-sm font-semibold text-[var(--ink-soft)]">募集中の求人({{ $jobPostings->count() }} 件)</h2>
            <div class="mt-3 grid gap-3 sm:grid-cols-2">
