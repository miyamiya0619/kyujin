...
@section('content')
    <section class="mx-auto max-w-xl px-4 py-8">
        <a href="{{ route('public.jobs.show', $jobPosting) }}" class="text-sm text-gray-600 hover:underline">&laquo; 求人詳細へ戻る</a>

        <h1 class="mt-2 text-xl font-bold">この求人に応募する</h1>
        <p class="mt-1 text-sm text-gray-600">{{ $jobPosting->title }} / {{ $jobPosting->company->name }}</p>
        ...
        @if ($alreadyApplied)
            <div class="mt-6 rounded border border-gray-200 bg-white p-6 text-sm">
