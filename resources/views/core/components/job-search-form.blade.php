...
<form method="GET" action="{{ route('public.jobs.index') }}" class="rounded border border-gray-200 bg-white p-4">
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div>
            <label for="prefecture_id" class="block text-xs text-gray-600">都道府県</label>
        ...
        <div>
            <label for="city_id" class="block text-xs text-gray-600">市区町村</label>
        ...
        @if ($jobCategories)
            <div>
                <label for="job_category_id" class="block text-xs text-gray-600">職種</label>
        ...
        @if ($employmentTypes)
            <div>
                <label for="employment_type_id" class="block text-xs text-gray-600">雇用形態</label>
    ...
    @unless ($compact)
        <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            @if ($facilityTypes)
                <div>
                    <label for="facility_type_id" class="block text-xs text-gray-600">施設形態</label>
            ...
            <div>
                <label for="keyword" class="block text-xs text-gray-600">キーワード</label>
        ...
        @if ($jobFeatures)
            <div class="mt-3">
                <p class="text-xs text-gray-600">こだわり条件</p>
    ...
    <button type="submit" class="mt-4 rounded px-6 py-2 text-sm font-semibold text-white"
