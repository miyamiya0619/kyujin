...
@section('content')
    ...
    <p class="mt-1 text-sm text-[var(--ink-soft)]">変更は保存すると即座にサイト全体へ反映されます。</p>
    ...
    <form method="POST" action="{{ route('admin.site-settings.update') }}" enctype="multipart/form-data"
        ...
        <div class="rounded border border-[var(--border)] bg-[var(--surface)] p-6">
            <h2 class="text-sm font-semibold text-[var(--ink-soft)]">メディアの基本情報</h2>
                ...
                <x-form.field name="key_visual" label="キービジュアル" help="トップページ等で使う画像です。JPEG・PNG・WebP、4MB まで。">
                    @if ($siteSetting->key_visual_path)
                        <img src="{{ \App\Services\ImageUploadService::url($siteSetting->key_visual_path) }}"
                             alt="現在のキービジュアル" class="mt-2 h-24 rounded border border-[var(--border)] bg-[var(--surface)] p-1">
                    @endif
        ...
        <div class="rounded border border-[var(--border)] bg-[var(--surface)] p-6">
            <h2 class="text-sm font-semibold text-[var(--ink-soft)]">見た目</h2>
        ...
        <div class="rounded border border-[var(--border)] bg-[var(--surface)] p-6">
            <h2 class="text-sm font-semibold text-[var(--ink-soft)]">連絡先</h2>
        ...
        <div class="rounded border border-[var(--border)] bg-[var(--surface)] p-6">
            <h2 class="text-sm font-semibold text-[var(--ink-soft)]">機能のオンオフ</h2>

            <div class="mt-4 space-y-3 text-sm">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="requires_review" value="1"
                           @checked(old('requires_review', $siteSetting->requires_review)) class="rounded border-[var(--border)]">
                    審査フロー(OFF にすると掲載企業の提出が即公開になります)
                </label>

                <label class="flex items-center gap-2">
                    <input type="checkbox" name="enables_member" value="1"
                           @checked(old('enables_member', $siteSetting->enables_member)) class="rounded border-[var(--border)]">
                    求職者会員機能(OFF にすると都度応募のみになります)
                </label>

                <label class="flex items-center gap-2">
                    <input type="checkbox" name="enables_posting_plan" value="1"
                           @checked(old('enables_posting_plan', $siteSetting->enables_posting_plan)) class="rounded border-[var(--border)]">
                    掲載プランによる掲載件数の制限
                </label>

                <label class="flex items-center gap-2">
                    <input type="checkbox" name="enables_external_feed" value="1"
                           @checked(old('enables_external_feed', $siteSetting->enables_external_feed)) class="rounded border-[var(--border)]">
        ...
        <div class="rounded border border-[var(--border)] bg-[var(--surface)] p-6">
            <h2 class="text-sm font-semibold text-[var(--ink-soft)]">規約類</h2>
