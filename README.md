...
顧客ごとに Xserver を 1 契約し、同じコードを git 経由で配布する(1環境 = 1メディア)。

| ドキュメント | 内容 |
|---|---|
| [docs/新規環境の構築手順.md](docs/新規環境の構築手順.md) | 新しい顧客環境を 1 つ立ち上げる手順 |
| [docs/デプロイ手順.md](docs/デプロイ手順.md) | 既存環境へコード更新を配布する手順(`deploy.sh`) |
| [docs/環境台帳.md](docs/環境台帳.md) | 顧客名・ドメイン・バージョン・最終デプロイ日の一覧 |
| [docs/解約時のデータ返還手順.md](docs/解約時のデータ返還手順.md) | 解約時の全データエクスポート・完全削除(`data:export` / `data:destroy-all`) |
| [docs/Xserver実機調査.md](docs/Xserver実機調査.md) | 新規環境ごとに最初に行う実機確認 |

`deploy.sh` / `backup.sh` はリポジトリ直下にある。本番では `chmod +x` してから使う
