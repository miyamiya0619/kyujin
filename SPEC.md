...
| 種別 | ガード | ログイン URL | 管理画面 |
|---|---|---|---|
| 運営者 | `admin` | `/admin/login` | `/admin` |
| 掲載企業 | `company` | `/company/login` | `/company` |
| 求職者 | `seeker` | `/login`(公開サイト内) | `/mypage` |

**ミドルウェアでは必ずガードを明示する**(`auth:admin` / `auth:company` / `auth:seeker`)。
無指定にすると既定ガード(`seeker`)が使われ、権限の穴になる。

ガード名が違えばセッションのキーも分かれるため、**同じブラウザで 3 つ同時にログインできる**。
運営者が動作確認するときに便利。ログアウトは対象ガードのみを落とし、他のガードのログイン状態は維持する。

#### パスワード再設定のトークンは種別ごとに別テーブル

`admin_password_reset_tokens` / `company_password_reset_tokens` / `job_seeker_password_reset_tokens` の 3 つに分ける。

Laravel 既定のトークンテーブルはメールアドレスが主キーのため、1 つを共有すると
**同じメールアドレスを持つ運営者と求職者が同時に申請したときに互いのトークンを上書きしてしまう**。
一人が複数の立場を持つことは実際に起こりうる(運営スタッフが求職者として登録するなど)。

なお、パスワード再設定メールは**意図的にキューに載せていない**。
本番のキューは cron から毎分実行するため、キューに載せると最大 1 分遅れる。
再設定は待たされると体験が悪く、その場で届くことのほうが重要。
...
**会員登録とメール認証(T-27)**: 会員登録直後は自動ログインするが、マイページ・
プロフィール編集など主要機能はメール認証が完了するまで案内画面に止まる。
**応募フォームからの応募はこの制限の対象外**(未ログインの求職者が会員登録と
同時に応募できる導線を壊さないため)。認証リンクの有効期限は24時間。
24時間以内に認証されず、かつ応募実績も無い仮登録は日次バッチで自動削除し、
同じメールアドレスで再登録できるようにする(`job-seekers:purge-unverified`)。
...
**メール認証が済むまではマイページに入れず、認証案内画面(再送ボタン付き)に
止まる**(5.1参照)。退会はメール認証の有無にかかわらずできる。
...
**全国 1,747 件を製品に同梱している。** 顧客環境でも通常の `db:seed` だけで全件入り、手作業の取り込みは不要。

データの実体は `database/data/cities.php`(総務省「全国地方公共団体コード」から生成した PHP 配列)。
`CitySeeder` がこれを読み込む。

```bash
# 市区町村が合併・分割されたとき、公式ファイルを再取得して作り直す
php artisan masters:build-city-data path/to/000925835.xlsx
```

xlsx は Excel を追加ライブラリなしで読む(ZIP 内の XML を直接パースする)。
`phpspreadsheet` をこの 1 箇所のためだけに入れていない。

公式データ: https://www.soumu.go.jp/denshijiti/code.html

**含まれないもの**: 政令指定都市の行政区(札幌市中央区など)。公式データにも含まれないため。
求人検索では「札幌市」までの絞り込みになる。東京 23 区は特別区として個別に含まれる。
